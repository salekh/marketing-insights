...
def generate_newsletter_html(
    ...
    """Generates the final REWE marketing newsletter as an HTML document using the official REWE HTML template.

    Automatically populates the template placeholders based on generated image, blog text, and product recommendations.
    If any content is missing, it invents realistic REWE defaults.

    Args:
        customer_name: First name of the customer (e.g., 'Sanchit' or 'Kunde').
        headline: Main hero headline for the newsletter (e.g., 'Deine Angebote\\nfür festliche\\nMomente!').
        badge_text: Circular badge greeting text (e.g., 'Wir\\nwünschen\\nschöne\\nFesttage').
        body_text: The blog post / introductory text body (~300 words).
        hero_image_url: URL of the hero image generated by generate_blog_image.
        hero_image_alt: Alt text description of the hero image.
        cta_button_text: Button CTA label (default 'Jetzt zugreifen & sparen').
        valid_until_text: Offer validity date text (default 'Gültig bis 27.12.2025').
        products: List of recommended products (from get_product_recommendations). Each item can contain 'name', 'description' (or 'usage_tip'), 'price', 'image_url', 'badge', and 'discount_pill'.
...
def generate_newsletter_a2ui(
    ...
    """Generates the final REWE marketing newsletter as an A2UI v0.9 createSurface JSON payload.

    Automatically populates the template placeholders based on generated image, blog text, and product recommendations.
    If any content is missing, it invents realistic REWE defaults.
    The returned JSON payload represents structured UI components without large HTML payloads.
    ...
    Returns:
        The compact A2UI JSON payload string for rendering the newsletter.
    """
    ...
    envelope_list = render_newsletter_a2ui(
        customer_name=customer_name,
        headline=headline,
        badge_text=badge_text,
        body_text=body_text,
    ...
    try:
        ...
        # Extract tool_context if passed via kwargs or frame inspection
        import inspect
